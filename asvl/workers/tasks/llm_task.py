...
def process_llm(self, task_id: str, asr_result: dict = None):
    """
    LLM处理任务

    Args:
        task_id: 任务ID
        asr_result: ASR结果（可选，如果不提供则从数据库读取）

    Returns:
        dict: 处理结果
    """
    log.info(f"Starting LLM task for {task_id}")
    return asyncio.run(_process_llm_async(self, task_id, asr_result))
...
async def _process_llm_async(
    ...
    try:
        # 1. 更新任务状态
        await _update_task_progress(task_id, "llm", TaskStatus.PROCESSING)

        # 2. 获取ASR结果
        if not asr_result:
            asr_result = await _get_asr_result(task_id)
        ...
        # 10. 更新任务进度
        await _update_task_progress(task_id, "llm", TaskStatus.COMPLETED)
        ...
        await _update_task_progress(task_id, "llm", TaskStatus.FAILED)
        raise


async def _get_asr_result(task_id: str) -> dict:
    """从数据库获取ASR结果"""
...
def get_segment_result(task_id: str) -> dict:
    """获取分段处理结果"""
    return asyncio.run(_get_segment_result_async(task_id))
